%\VignetteIndexEntry{Example data dictionary using queries from the SQL repository}
\documentclass[letterpaper,11pt,nogin]{article}
...
sqlPaths(paste(system.file(package='sqlutils'), '/data', sep=''), replace=TRUE)
queries <- getQueries()
nlevels <- 10
...
This ``vignette" is a template for building a data dictionary based upon the all the queries returned by \texttt{getQueries}. This Sweave file can be run using your own library of quries as defined in \texttt{sqlPaths}. 

<<docs,echo=FALSE,results=tex>>=
